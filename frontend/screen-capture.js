...
class InventoryAnalyzer {
    ...
    async captureScreen() {
        try {
            ...
            // Save the image using Jimp
            const image = await Jimp.read(Buffer.from(thumbnail.split(',')[1], 'base64'));

            // If we have a specific inventory region, crop the image
            if (this.config.inventoryRegion) {
                const { x, y, width, height } = this.config.inventoryRegion;
    ...
    // New method to detect grid patterns in the inventory
